...
public class ActionForControlPlayer : MonoBehaviour {
    ...
    // ----------------------------- //
    public void MoveRightStick(InputAction.CallbackContext context)
    {
        ...
        {
            aimWithStick = true;
            changeRotation?.Invoke();
        }
        if (context.phase == InputActionPhase.Performed) _rightStick = _playerInput.actions["Aim"].ReadValue<Vector2>();
        if (context.phase == InputActionPhase.Canceled) aimWithStick = false;
    }
    public void MoveLeftStick(InputAction.CallbackContext context)
    {
        ...
        {
            _leftStick = _playerInput.actions["Move"].ReadValue<Vector2>();
            if (!aimWithStick)
            {
                _rightStick = _playerInput.actions["Move"].ReadValue<Vector2>();
                changeRotation?.Invoke();
            }
        }
    }
    ...
    {
        // CRÍTICO: No permitir pausar durante loading
    ...
    public Vector2 RightStick() { return _rightStick; }
