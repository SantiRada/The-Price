...
public class RoomManager : MonoBehaviour {
    ...
    [Tooltip("Se debe ordenar desde el que menos posible sea que aparezca, al que m�s")] public GameObject[] typeChest;
    ...
    private void ResetValuesToNewRoom()
    {
        ...
        // VERIFICA OBJETOS EN ESCENA PARA ELIMINARLOS ANTES DE LA CREACI�N DE UNA NUEVA SALA
        ...
        // ELIMINA TODOS LOS OBJETOS DE DA�O
    ...
    private IEnumerator CreateRoom()
    {
        ...
        // --- CREAR SALA SEG�N ESPACIO ACTUAL --- //
        ...
        #region TEXT PER ROOM --PROVISIONAL--
        ...
        // Rezising, Reposition & Load from Room
        walkableMap.SizeMap = currentRoom.mapSize;
        walkableMap.InitialPosition = currentRoom.spawnMap.transform.position;
        walkableMap.walkableMap = currentRoom.GetWalkableMap2D();
        walkableMap.CompleteMap();
        ...
        if (canCreateChest)
        {
            if (_typeRooms[_countRoomsComplete] != TypeRoom.MiniBoss && _typeRooms[_countRoomsComplete] != TypeRoom.Boss && _typeRooms[_countRoomsComplete] != TypeRoom.Shop)
            {
                ...
                // BUSCAR UNA POSICI�N WALKABLE ALEATORIA PARA COLOCAR EL COFRE
                do
                {
                    x = UnityEngine.Random.Range(0, currentRoom.sizeMap.x);
                    y = UnityEngine.Random.Range(0, currentRoom.sizeMap.y);
                    isWalkable = walkableMap.GetPositionIsWalkable((int)x, (int)y);
                } while (!isWalkable);

                x += walkableMap.transform.position.x;
                y += walkableMap.transform.position.y;
    ...
    private void SetTypeRoomForThisPlace()
    {
        ...
        // Asignar salas aleatorias restantes a Oro y B�sica
